...
GPU::GPU(Core::System& system, Frontend::EmuWindow& emu_window,
    ...
    impl->p3d_interrupt_event =
        impl->timing.RegisterEvent("GPU::P3DInterruptCallback", [this](uintptr_t user_data,
                                                                       s64 cycles_late) {
            P3DInterruptCallback(user_data, cycles_late);
        });
...
void GPU::SetInterruptHandler(Service::GSP::InterruptHandler handler) {
    ...
    Service::GSP::InterruptHandler pica_interrupt_handler = [this](Service::GSP::InterruptId id) {
        if (id == Service::GSP::InterruptId::P3D) {
            ScheduleP3DInterrupt();
            return;
        }

        if (impl->signal_interrupt) {
            impl->signal_interrupt(id);
        }
    };
    impl->pica.SetInterruptHandler(pica_interrupt_handler);
}
